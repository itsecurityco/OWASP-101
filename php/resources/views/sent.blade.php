            ...
            <table class="table table-secondary table-striped table-md">
                <thead>
                    <tr>
                        ...
                        <th class="text-start">Beneficiary Bank</th>
...
<script type="text/javascript">
    ...
    .then(result => {
        ...
        if (! result.length){
            ...
            html += 'Your account has not sent transfers yet'
            html += '</div>'
            ...
            result.forEach(transfer => {
                ...
                html += `<td class='text-start'>${transfer.destination_name}</td>`
                html += `<td class='text-start'>${transfer.destination_bank}</td>`
                ...
                html += `<td colspan='4' class='text-start text-muted align-middle bg-light'>`
                html += `<a href="/api/transfer/${transfer.id}">ID Transfer ${transfer.id}</a>`
                html += `</td></tr>`
            })
